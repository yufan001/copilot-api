/* eslint-disable complexity, max-lines-per-function, max-depth, eqeqeq, no-nested-ternary, @typescript-eslint/no-unnecessary-condition */
import {
 ...
 */
function isFileId(data: string, prefixes?: ReadonlyArray<string>): boolean {
...
export async function convertToOpenAIResponsesInput({
  ...
  fileIdPrefixes?: ReadonlyArray<string>
  store: boolean
      ...
      case "system": {
        switch (systemMessageMode) {
          ...
          default: {
            ...
            throw new Error(
              `Unsupported system message mode: ${String(_exhaustiveCheck)}`,
            )
          }
        }
        break
          ...
          content: content.map((part, index) => {
            switch (part.type) {
              ...
              default: {
                ...
                throw new Error(
                  `Unsupported user content part: ${String(_exhaustiveCheck)}`,
                )
              }
            }
          }),
      ...
      case "assistant": {
        ...
        for (const part of content) {
          switch (part.type) {
            ...
            default: {
              throw new Error(
                `Unsupported assistant content part: ${part.type}`,
              )
            }
          }
        }
        ...
        for (const part of content) {
          ...
          switch (output.type) {
            ...
            default: {
              ...
              throw new Error(
                `Unsupported tool output type: ${String(_exhaustiveCheck)}`,
              )
            }
          }
